...
public class Xiaoxi extends Activity  implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xiaoxiinfo);
		setTitle("消息中心");
		
		pd = ProgressDialog.show(this, "请稍候..", "正在获取消息信息...", true,	false);
		
		new Thread(){
			@Override
			public void run(){
				//你要执行的方法
				feed = DataToolsHelper.getXiaoxiRss();
				//执行完毕后给handler发送一个消息
	...
	private void showNullListView() {
		...
		tv.setText("无法连接到服务器");
		RelativeLayout layout = (RelativeLayout) this.findViewById(R.id.xiaoxilayout);
		layout.addView(tv);
	}
	...
	private void showListView() {
		...
		ListView itemList = (ListView) this.findViewById(R.id.xiaoxilist);
		if (feed == null) {
			setTitle("访问失败");
			return;
		}
